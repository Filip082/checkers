...
public class Move {
    ...
    private Move(Board board, Position origin, Position destination, Move prev) throws InvalidMoveException {
        if (origin == null || destination == null) {
            throw new InvalidMoveException(origin == null ? "Origin" : "Destination" + " is null");
        }
        movingPawn = prev != null ? prev.getMovingPawn() : board.getSquare(origin).getPawn();
        if (movingPawn == null || !board.getSquare(destination).isEmpty()) {
    ...
    private Move(Board board, List<Position> moveSequence, Move prev) throws InvalidMoveException {
        this(board, moveSequence.removeFirst(), !moveSequence.isEmpty() ? moveSequence.getFirst() : null, prev);
        if (moveSequence.size() > 1) {
            if (this.captured == null)
                throw new InvalidMoveException("");
            nextMove = new Move(board, moveSequence, this);
            if (nextMove.getCaptured().isEmpty())
                throw new InvalidMoveException("");
        }
    }

    public Move(Board board, List<Position> moveSequence) throws InvalidMoveException {
        this(board, moveSequence, null);
    }
